...
def generate_translation_dict_partial(model, version, is_multimer=False):
    #######################
    # Some templates
    #######################
    LinearWeight = lambda l: (Param(l, param_type=ParamType.LinearWeight))
...
def import_jax_weights_(model, npz_path, partial=False, version="model_1"):
    data = np.load(npz_path)
    if partial:
        translations = generate_translation_dict_partial(model, version, is_multimer=("multimer" in version))
    else:
