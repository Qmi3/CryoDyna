...
import cryostar.openfold.utils.residue_constants as rc
from cryostar.openfold.utils.geometry import rigid_matrix_vector, rotation_matrix, vector
...
def atom14_to_atom37(atom14, batch):
    atom37_data = batched_gather(
        atom14,
        torch.stack([batch["residx_atom37_to_atom14"]] * atom14.shape[0]).to(atom14.device),
        dim=-2,
        no_batch_dims=len(atom14.shape[:-2]),
    )
    atom37_data = atom37_data * batch["atom37_atom_exists"][..., None].to(atom14.device)
    return atom37_data
