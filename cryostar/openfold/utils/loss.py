...
from cryostar.openfold.utils import residue_constants
from cryostar.openfold.utils.rigid_utils import Rotation, Rigid
from cryostar.openfold.utils.geometry.vector import Vec3Array, euclidean_distance
from cryostar.openfold.utils.all_atom_multimer import get_rc_tensor
...
def between_residue_bond_loss(
    ...
    c_n_bond_length_error = torch.sqrt(eps + (c_n_bond_length - gt_length.to(c_n_bond_length.device)) ** 2)
    c_n_loss_per_residue = torch.nn.functional.relu(
        c_n_bond_length_error - tolerance_factor_soft * gt_stddev.to(c_n_bond_length.device)
    )
