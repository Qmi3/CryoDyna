...
from cryostar.openfold.model.primitives import Linear, LayerNorm, ipa_point_weights_init_
from cryostar.openfold.utils.residue_constants import (
    restype_rigid_group_default_frame,
    restype_atom14_to_rigid_group,
    restype_atom14_mask,
    restype_atom14_rigid_group_positions,
)
from cryostar.openfold.utils.geometry.quat_rigid import QuatRigid
from cryostar.openfold.utils.geometry.rigid_matrix_vector import Rigid3Array
from cryostar.openfold.utils.geometry.vector import Vec3Array, square_euclidean_distance
from cryostar.openfold.utils.feats import (
    frames_and_literature_positions_to_atom14_pos,
    torsion_angles_to_frames,
)
from cryostar.openfold.utils.precision_utils import is_fp16_enabled
from cryostar.openfold.utils.rigid_utils import Rotation, Rigid
from cryostar.openfold.utils.tensor_utils import (
    dict_multimap,
    permute_final_dims,
    flatten_final_dims,
)
...
class InvariantPointAttentionMultimer(nn.Module):
    ...
    def __init__(
        ...
        self.linear_b = Linear(self.c_z, self.no_heads)
        # self.z = z_new
        # self.b_precompute = self.linear_b(self.c)
        self.head_weights = nn.Parameter(torch.zeros((no_heads)))
        ipa_point_weights_init_(self.head_weights)
    ...
    def forward(
        ...
        a = a + square_mask.unsqueeze(-1).to(a.device)
        # a = a + d
        ...
        o_pair = torch.einsum('...ijh, ...ijc->...ihc', a, z.to(dtype=a.dtype).to(a.device))
...
class StructureModule(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.inf = inf
        self.is_multimer = is_multimer
        self.single_repr = fold_feature['single']
        self.pair_repr = fold_feature['pair']
        self.mask = fold_feature['seq_mask']
        self.aatype = fold_feature["aatype"]
    ...
    def _forward_multimer(
        ...
        if mask is None:
            ...
            mask = s.new_ones(s.shape[:-1])
        # [*, N, C_s]
        # s = self.layer_norm_s(self.single_repr.to(s_delta.device) + s_delta)
        s = self.layer_norm_s(self.single_repr.to(s_delta.device))
        # [*, N, N, C_z]
        # z = self.layer_norm_z(evoformer_output_dict["pair"])
        z = self.layer_norm_z(self.pair_repr.to(s_delta.device))
        ...
        s_initial = s
        s = self.linear_in(s)
        # s = torch.stack([s]*d.shape[0])
        # z = torch.stack([z]*d.shape[0])
    ...
    def forward(
        ...
        if(self.is_multimer):
            outputs = self._forward_multimer(d, self.aatype, self.mask, inplace_safe, _offload_inference)
        else:
            outputs = self._forward_monomer(d, self.aatype, self.mask, inplace_safe, _offload_inference)
