...
AA3     = FUNC.spl("TRP TYR PHE HIS HIH HSD ARG LYS CYS ASP GLU ILE LEU MET ASN PRO HYP GLN SER THR VAL ALA GLY"+dnares3+rnares3) #@#
AA1     = FUNC.spl("  W   Y   F   H   H   H   R   K   C   D   E   I   L   M   N   P   O   Q   S   T   V   A   G"+dnares1+rnares1) #@#
...
nucleic = FUNC.spl("DAD DCY DGU DTH ADE CYT GUA THY URA DA DC DG DT A C G U")
...
class CoarseGrained:
    ...
    dna_bb = FUNC.nsplit("P OP1 OP2 O5' O3'","C5' O4' C4'","C3' C2' C1'")
    rna_bb = FUNC.nsplit("P OP1 OP2 O5' O3'","C5' O4' C4'","C3' C2' O2' C1'")
    ...
    mapping = {
        ...
        "DT": FUNC.nsplit("P OP1 OP2 O5' H5T O3' H3T O1P O2P",
                          ...
                          "N3 C2 O2",
                          "C5 C4 O4 C7 C5M"),
        "A":  FUNC.nsplit("P OP1 OP2 O5' H5T O3' H3T O1P O2P",
                          "C5' O4' C4'",
                          "C3' C2' O2' C1'",
                          "N9 C4",
                          "C2 N3",
                          "C6 N6 N1",
                          "C8 N7 C5"),
        "G":  FUNC.nsplit("P OP1 OP2 O5' H5T O3' H3T O1P O2P",
                          "C5' O4' C4'",
                          "C3' C2' O2' C1'",
                          "N9 C4",
                          "C2 N2 N3",
                          "C6 O6 N1",
                          "C8 N7 C5"),
        "C":  FUNC.nsplit("P OP1 OP2 O5' H5T O3' H3T O1P O2P",
                          "C5' O4' C4'",
                          "C3' C2' O2' C1'",
                          "N1 C6",
                          "N3 C2 O2",
                          "C5 C4 N4"),
        "U":  FUNC.nsplit("P OP1 OP2 O5' H5T O3' H3T O1P O2P",
                          "C5' O4' C4'",
                          "C3' C2' O2' C1'",
                          "N1 C6",
                          "N3 C2 O2",
    ...
    residue_bead_names_rna = FUNC.spl("BB1 BB2 BB3 SC1 SC2 SC3 SC4")
    ...
    electrons = {'H': 1, 'C': 6, 'N': 7, 'O': 8, 'S': 16, 'P': 15, 'M': 0}
...
def aver(b):
    mwx, es, ids = zip(*[((m*x, m*y, m*z), e, i) for m, (x, y, z), e,i in b])      # Weighted coordinates
    tm = sum([m[0] for m in b])    
    # Sum of weights
    # return [sum(i)/tm for i in zip(*mwx)], [sum(es)], [CoarseGrained.bead_sizes[len(b)]], ids                            # Centre of mass                          # Centre of mass
    return [sum(i)/tm for i in zip(*mwx)], [sum(es)], ids   
...
def map(r, ca2bb = False):
    ...
    a = [(i[0], CoarseGrained.mass.get(i[0][0], 0), CoarseGrained.electrons.get(i[0][0], 0), i[4:]) for i in r]
    # Store weight, coordinate and index for atoms that match a bead
    q = [[(m, coord, e, a.index((atom, m, e, coord))) for atom, m, e, coord in a if atom in i] for i in p]
    # Bead positions
    return zip(*[aver(i) for i in q if i])
