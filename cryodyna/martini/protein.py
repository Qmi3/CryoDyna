...
electrons = {'H': 0, 'C': 6, 'N': 7, 'O': 8, 'S': 16, 'P': 15, 'M': 0}
...
class Protein:
  ...
  num_electrons: np.ndarray  # [num_res, num_atom_type]
...
def _from_bio_structure(
  ...
  for chain in model:
    ...
    for res in chain:
      ...
      n_electrons = np.zeros((residue_constants.atom_type_num,))
      for atom in res:
        ...
        n_electrons[residue_constants.atom_order[atom.name]] = electrons[atom.element]
      if np.sum(mask) < 0.5:
      ...
      num_electrons.append(n_electrons)
