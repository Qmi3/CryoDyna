...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        pe_vector=None,
        beads_ids=[],
        pe_vector_res_2_meta=None,
        pe_vector_bead_2_res=None,
        is_CG=False,
    ):
        super().__init__()
        if encoder_cls == "MLP":
        ...
        if decoder_cls == "MLP":
            ...
            if is_CG:
                assert beads_ids != [], "beads_ids is required for bead-level model"
                assert pe_vector_res_2_meta is not None and pe_vector_bead_2_res is not None, "pe_vector_res_2_meta and pe_vector_bead_2_res are required for bead-level model"
                self.decoder = HierarchicalDeltaGNN_CG(in_dim = z_dim, latent_dim = latent_dim, sec_ids=sec_ids, beads_ids = beads_ids,meta_edge_index=meta_edge_index, 
                ...
                assert pe_vector is not None, "pe_vector is required for residue-level model"
                self.decoder = HierarchicalDeltaGNN(in_dim = z_dim, latent_dim = latent_dim, sec_ids=sec_ids, meta_edge_index=meta_edge_index, 
