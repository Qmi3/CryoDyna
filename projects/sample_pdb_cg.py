...
for i in range(0, len(z_all), batch_size):
    if i + batch_size > len(z_all):
        sampled_z_index = np.arange(i,len(z_all))
    else:
        sampled_z_index = np.arange(i,i+batch_size)
    # if i==128:
    #     import pdb; pdb.set_trace()
    sampled_z = z_all[sampled_z_index]
    with torch.no_grad():
    ...
    for j, z_idx in enumerate(sampled_z_index):
        ...
        tmp_struc = pred_struc[j].cpu().numpy()
