...
class InitTask(pl.LightningModule):
    ...
    def __init__(self, em_module):
        ...
        self.translator = SpatialGridTranslate(D=em_module.cfg.data_process.down_side_shape, device=self.device)
...
class CryoEMTask(pl.LightningModule):
    ...
    def __init__(self, cfg, dataset):
        ...
        # ref extract CA amps = num_eletron 
        ...
        # centroid_distances = cdist(ref_centers, ref_centers)
        ...
        # if cfg.model.model_cfg.decoder_cls == "MLP":
        self.model = VAE(in_dim=in_dim,
                        ...
                        **cfg.model.model_cfg)
        # elif cfg.model.model_cfg.decoder_cls == "metaGNN": 
        #     self.model = VAE(in_dim=in_dim,
        #                  out_dim=num_pts * 3 if nma_modes is None else 6 + nma_modes.shape[1],
        #                  sec_ids,
        #                  **cfg.model.model_cfg)
        ...
        inter_cutoff_pairs = find_quaint_cutoff_pairs(meta.coord, meta.chain_id,
                                                           meta.res_id, 0,
                                                           cfg.loss.inter_chain_cutoff, cfg.loss.intra_chain_res_bound)
        ...
        if len(inter_cutoff_pairs) > 0:
            dists = calc_dist_by_pair_indices(meta.coord, inter_cutoff_pairs)
            log_to_current(f"found {len(inter_cutoff_pairs)} inter cutoff_pairs")
            self.inter_dist_loss_fn = DistLoss(inter_cutoff_pairs, dists, reduction=None)
        else:
            log_to_current("inter cutoff_pairs is empty")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if torch.isnan(batch["proj"]).any() or torch.isinf(batch["proj"]).any():
        #     raise ValueError("pred_latent contains NaNs or infs!")
        # if torch.isnan(mu).any() or torch.isinf(mu).any():
        #     raise ValueError("pred_latent contains NaNs or infs!")
        # if torch.isnan(pred_struc).any() or torch.isinf(pred_struc).any():
        #     raise ValueError("pred_struc contains NaNs or infs!")
        # if torch.isnan(pred_gmm_images).any() or torch.isinf(pred_gmm_images).any():
        #     raise ValueError("pred_images contains NaNs or infs!")
        ...
        if hasattr(self, "inter_dist_loss_fn"):
            inter_dist_loss = self.inter_dist_loss_fn(pred_struc)
            # across devices
            all_inter_dist_loss = self.all_gather(inter_dist_loss)  # world_size, batch, num_pairs
            all_inter_dist_loss = all_inter_dist_loss.reshape(-1, inter_dist_loss.shape[-1])
            with torch.no_grad():
                inter_var = all_inter_dist_loss.var(dim=0)
                inter_chain_keep_mask = inter_var.lt(torch.quantile(inter_var, cfg.loss.inter_dist_keep_ratio))
                # keep_mask[tmp_mask] *= intra_chain_keep_mask

                inter_chain_keep_mask = inter_chain_keep_mask.unsqueeze(0).repeat(inter_dist_loss.size(0), 1)
            inter_dist_loss = torch.mean(inter_dist_loss[inter_chain_keep_mask])
            weighted_inter_dist_loss = cfg.loss.inter_dist_weight * inter_dist_loss

            # dist_penalty = torch.mean(torch.abs(self.dist_loss_fn.get_weights()))
            # weighted_dist_penalty = cfg.loss.dist_penalty_weight * dist_penalty
        else:
            weighted_inter_dist_loss = weighted_gmm_proj_loss.new_tensor(0.)
            # weighted_dist_penalty = weighted_gmm_proj_loss.new_tensor(0.)
        ...
        # clac loss
        loss = (weighted_gmm_proj_loss + weighted_connect_loss + weighted_dist_loss +
        ...
        tmp_metric = {
            ...
            "inter_dist":weighted_inter_dist_loss.item(),
        ...
        self.training_step_outputs.append({"cryoem(gmm)": tmp_metric['cryoem(gmm)'], "con":tmp_metric['con'], "clash":tmp_metric['clash'] ,
                                             "dist":tmp_metric['dist'], "inter_dist":tmp_metric['inter_dist'], "sse":tmp_metric['sse']})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        z = self.model.encode(prepare_images(gt_images, self.cfg.model.input_space))
        # z = mu
