...
def call_dssp(chain,atomlist,executable='dsspcmbi'):
    ...
    ssdfile = '%s/chain_%s.ssd'%(os.path.dirname(options['-o'].value),chain.id)
    try:
...
def main(options):
    ...
    for title,atoms,box in frameIterator(inStream):
        ...
        if fileType == "PDB":
            ...
            #     residuelist = [residue for residue in residues(chain)]
            #     chainID = residuelist[0][0][3]
            #     broken = breaks(residuelist)
            #     chains_ = zip([0]+broken,broken+[len(residuelist)],range(len(broken)+1))
            #     if len(chains_) > 1:
            #         chains += [ Chain(options,residuelist[i:j],name=chainID+str(k)) for i,j,k in chains_ ]
            #     else:
            #         chains += [ Chain(options,residuelist,name=chainID) ]
        else:
    ...
    if options['-o']:
        ...
        for mi in range(len(molecules)):
            ...
            if not mol in moleculeTypes or options['SeparateTop']:
                ...
                destination = options["-o"] and open(os.path.join(os.path.dirname(options['-o'].value),moleculeTypes[mol]+".itp"),'w') or sys.stdout
        ...
        cmd = "cp %s/martini_v2.1-dna.itp %s/martini_v2.1-dna.itp" % (os.path.dirname(__file__), os.path.dirname(options['-o'].value))
        os.system(cmd)
        top.write(
